...
interface SheetsRuntimeConfig {
  ...
  /** Shell router used to open exported/AI-generated files in a new 方塘Office tab. */
...
async function createStandaloneSheetsDocument(
  ...
  if (request.type === 'docx') {
    return { ok: false, error: 'Creating DOCX files requires the 方塘Office shell or Docs app.' }
  }
...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle('ai:preset-catalog', () =>
    composePresetCatalog(
      BUILTIN_PRESETS,
      loadPresetFiles(bundledPresetsDir(), userDataPath('presets')),
    ),
  )

  // built-in MCP servers (fangtang-mcp.json): tools for the preset skills
  const mcpHub = new McpHub(loadMcpConfig(undefined, userDataPath('fangtang-mcp.json')).servers)
