...
function makeAccess(opts?: {
  ...
  const imagesSeen: string[][] = [] // records the images each page-generation call received
...
describe('generate_deck local page generation', () => {
  ...
  it('no local pipeline available → fails fast', async () => {
    const { access } = makeAccess()
...
describe('generate_deck lands pages while generating + retries', () => {
  ...
  it('a page failing beyond its inline retry gets one final-round regeneration, then is skipped', async () => {
    const { access, getPages, localPageCalls } = makeAccess({ failPages: [2] })
    const skill = createSlidesSkill(access)
    await skill.executeTool(deckCall(3))
    expect(getPages()).toBe(2)
    // Two inline attempts + two more in the retry round, then the page stays skipped
...
describe('generate_deck in-tool image search', () => {
  it('image_queries are keywords → tool searches images → real URLs passed to the page generator', async () => {
