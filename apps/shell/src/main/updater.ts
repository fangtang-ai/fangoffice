 ...
 * itself is injected at build time via the FANGTANG_UPDATE_URL env var and
 ...
 *
 * Dev preview: FANGTANG_FAKE_UPDATE=<version> in an unpacked run opens the
...
const DOWNLOAD_PAGE_URL = 'https://fang-tang.cn'
...
export function initAutoUpdater(
  ...
  if (!app.isPackaged && process.env.FANGTANG_FAKE_UPDATE) {
    initFakeUpdate(getWindow, process.env.FANGTANG_FAKE_UPDATE)
    return
  }
